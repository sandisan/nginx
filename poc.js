			...
			data.entry.forEach(function(item) {
				...
				$.ajax({
					...
					headers: {
						...
						var html = '<div class="card shadow mb-3 bg-white rounded"> <div class="card-body"> <div class="row"> <div class="col-lg-2 m-auto" ><img class="photo" src="images/photo.png" alt="" /></div> <div class="col "> <div class="row "> <div class="col align-center border-bottom col-prs-name" ><a href="#" id="prs-name" onclick=getpractitioner('+ p_id +');>'+ data.name[0].text + '</a><div class="pull-right text-info">Role: ' + item.resource.code[0].coding[0].code.charAt(0).toUpperCase() + item.resource.code[0].coding[0].code.slice(1) + '<div></div> </div> <div class="row"> <div class="col align-center col-prs" ><label class="prs-label">Specialty</label><br><input type="text" name="prs-spec" id="prs-spec" value="'+ item.resource.specialty[0].coding[0].display +'" readonly class="prs-input"></div> <div class="col align-center col-prs" ><label for="prs-sdate" class="prs-label">Location</label><div>'+item.resource.location[0].display +'</div></div> <div class="col align-center col-prs" ><label  class="prs-label">HealthCare Service</label><br><div>'+item.resource.healthcareService[0].display +' </div></div> </div> </div> </div> </div> </div>';
						$("#practitioners_div").append(html);	
						});
					});
					}).fail(function(data){
